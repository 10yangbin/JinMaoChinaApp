...
View.OnClickListener {
	...
	private void initData() {
		...
		if(returnKeyNotiece !=null){
			Toast.makeText(LoginActivity.this, returnKeyNotiece, Toast.LENGTH_SHORT);
		}
	...
	private void initView() {
		ImageView ivQuestion = (ImageView) findViewById(R.id.ivQuestion);
		...
		if(!StringUtil.isNull(MainApplication.getInstance().getUserName())&&!StringUtil.isNull(MainApplication.getInstance().getPassword())){
			etUserName.setText(MainApplication.getInstance().getUserName());
			etPwd.setText(MainApplication.getInstance().getPassword());
		}
	}
	...
	private void doLogin() {
		if (checkInput()) {
			...
			try {
				ubiz.login(etUserName.getText().toString().toUpperCase(), etPwd
						.getText().toString(), new OperateCallBack() {
	...
	private boolean checkInput() {
		...
		if (StringUtil.isNull(etUserName.getText().toString())) {
			Toast.makeText(this, "用户名不能为空", Toast.LENGTH_SHORT).show();
			return false;
		}

		if (StringUtil.isNull(etPwd.getText().toString())) {
