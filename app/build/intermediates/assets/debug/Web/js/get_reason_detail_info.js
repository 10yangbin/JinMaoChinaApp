			...
			function bsTypeShow(){
				uiFlag = 1;
				var sql = "select REASON_DETAIL as name, REASON_DETAIL_ID as id from REASON_CAUSE where REASON_MAIN_ID like '202101101%'";
			...
			function bsTypeShowDetailThrid(id){
				uiFlag = 8;
				var sql = "select REASON_DETAIL as name, REASON_DETAIL_ID as id ,REASON_MAIN_ID as key_id from REASON_CAUSE where REASON_DETAIL_ID like '202101101%'";//'"+id+"_________|'";//'GZ|"+id+"|________|'";
				var reasonData = window.csq_db.db_select(0,sql, '',1);
				reasonData = eval("("+reasonData+")");
				if(reasonData && reasonData.length>0){
					return true;
			...
			//三级数据 显示
			function bsTypeShowDetailThridShow(){
				uiFlag = 8;
				//alert(task_reason_main_id);
				var sql = "select REASON_DETAIL as name, REASON_DETAIL_ID as id ,REASON_MAIN_ID as key_id from REASON_CAUSE where REASON_MAIN_ID like '"+task_reason_main_id+"%' and REASON_MAIN_ID != '"+task_reason_main_id+"' ";
				var reasonData = window.csq_db.db_select(0,sql, '',1);
				reasonData = eval("("+reasonData+")");
